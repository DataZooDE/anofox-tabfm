# PATCHED COPY of vendor/tabfm/tabfm/src/pytorch/model.py (Apache-2.0).
# DO NOT EDIT the vendor submodule; this copy exists only for ONNX export.
#
# Exactly two lines differ from upstream (S01 RESULTS.md, "model.py patch"):
# both `repeat_interleave` calls are rewritten to mathematically identical
# expand+reshape, because the dynamo->ONNX translation has no conversion for
# aten.repeat_interleave.self_int ("No conversion available yet when dim is
# None"). Everything else (the % d gather, dynamic-arange RoPE, one_hot)
# exports unchanged. tests/test_patched_model.py asserts this file is the
# vendor file plus exactly this diff.
#
  ...
  def forward(self, x, d=None):  # x: [B,T,HC,E]
    ...
    if d is not None:
      ...
      mask = (valid[:, None, :].expand(b, t, hc)
              .reshape(b * t, hc))[:, None, None, :]  # [B*T, 1, 1, HC]
