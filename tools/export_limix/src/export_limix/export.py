"""LimiX -> weight-free ONNX graph + tensor-name map.

Mirrors ``tools/export_tabicl`` step for step (LimiX is a TabICL descendant),
...
# LimiX supports both tasks: classification feeds dense class ids, regression
# feeds RAW continuous targets (LimiX normalizes the target internally).


def example_inputs(t, h, s, max_classes, seed=0, task="classification"):
    torch.manual_seed(seed)
    x = torch.randn(1, t, h)
    if task == "regression":
        y = torch.randn(1, s) * 3.0 + 7.0
    else:
...
def export_graph(model, graph_path: pathlib.Path, *,
                 ...
                 example=(20, 5, 12), max_classes=10,
                 task="classification",
    ...
    dyn = ({1: T, 2: H}, {1: S})  # x, y
    t, h, s = example
    ex = example_inputs(t, h, s, max_classes, task=task)
...
# LimiX checkpoints store their state_dict in the BARE module namespace
# (verified against LimiX-2M.ckpt, e.g. "encoder_x.0.mask_embedding"), so no
# prefix is re-applied — unlike the torch.compile-wrapped Orion-BiX ckpt.
CKPT_KEY_PREFIX = ""
...
def make_feed(t, h, s, max_classes, seed=1, task="classification"):
    rng = np.random.default_rng(seed)
    x = rng.standard_normal((1, t, h)).astype(np.float32)
    if task == "regression":
        y = (rng.standard_normal((1, s)) * 3.0 + 7.0).astype(np.float32)
    else:
...
def check_parity(graph_path: pathlib.Path, wrapper: ExportWrapper,
    ...
    results, worst = [], 0.0
    for t, h, s in shapes:
        feed = make_feed(t, h, s, max_classes, task=task)
        t0 = time.time()
