"""CI fixture model for Orion-MSP — random weights, tiny dims, deterministic bytes.

Same shape as the TabICL fixture (test/fixtures/tabicl): a tiny random-init
OrionMSP (MIT architecture, OUR weights — zero Lexsi checkpoint bytes) with
...
**Single capability.** Orion-MSP ships no regression head, so the manifest
declares ``capabilities: ["classify"]`` — this is the first built-in that does
not support both tasks, and `test/sql/tabfm_orion_msp.test` asserts that
...
from export_orion_msp import configs as x_configs
from export_orion_msp import export as x_export
from export_orion_msp.orion_msp_patches import apply, build_model
...
def safetensors_bytes_digest(model, st_path: pathlib.Path) -> str:
    save_file(
        ...
        metadata={"origin": "anofox-tabfm CI fixture, random init, MIT Orion-MSP "
...
def build(out: pathlib.Path) -> dict:
    ...
    x_export.write_tensor_map(out / f"tensor_map_orion_msp_{TASK}.json", tensor_map,
    ...
    (out / f"golden_{TASK}.json").write_text(json.dumps({
        "_doc": {
            "purpose": "C++ parity: safetensors -> initializer injection -> ORT run on "
                       f"graph_orion_msp_{TASK}.onnx must reproduce these fp32 logits.",
    ...
    manifest = {
        ...
        "display_name": "Orion-MSP CI fixture (random init, schema v2)",
        "family": "icl-transformer",
        "license": {"id": "mit", "commercial": True,
                    "redistributable": True, "gate_setting": None},
        # NOT a "*_raw" profile: unlike TabPFN/TabICL, Orion-MSP does no internal
        ...
        "weights": {
            TASK: {"repo": "local:test/fixtures/orion_msp", "revision": "fixture-v1",
                   "files": [file_entry(f"model_{TASK}.safetensors")]},
        },
        "graph": {TASK: f"graph_orion_msp_{TASK}.onnx",
                  "tensor_map": f"tensor_map_orion_msp_{TASK}.json"},
        ...
        "compute": {"cpu": "f32"},
        "_note": "Random-init Orion-MSP fixture (MIT architecture, our weights). H is "
