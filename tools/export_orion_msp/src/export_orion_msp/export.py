"""Orion-MSP -> weight-free ONNX graph + tensor-name map.

Mirrors ``tools/export_tabicl`` step for step (Orion-MSP is a TabICL descendant),
...
from export_orion_msp.orion_msp_patches import ExportWrapper

OPSET = 18
PARITY_TOL = 1e-3  # budget

# Orion-MSP is classification-only (no upstream regression head), so y always
...
# The released Orion-MSP checkpoint was saved from a torch.compile-wrapped
# module, so every state_dict key carries an "_orig_mod." prefix (upstream's
# sklearn loader strips it: classifier.py `key.replace("_orig_mod.", "")`).
# The extension injects initializers by looking names up in the checkpoint AS
# SHIPPED — src/tabfm_ckpt.cpp unwraps the "state_dict" key but does not rewrite
# parameter names — so the tensor map must record the prefixed keys.
# Orion-MSP needs NO key prefix: the released OrionMSP-classifier-v1.5 checkpoint
# stores bare module names ("col_embedder.in_linear.weight", ...) — verified
# against the published .ckpt, where all 357 mapped names match exactly.
#
# (This is "_orig_mod." in tools/export_orion_bix, whose v1.1 checkpoint WAS
# saved from a torch.compile-wrapped module. Inheriting that value here mapped
# 0/357 keys and would have produced a graph that loads and injects nothing —
# the same trap tools/export_tabdpt hit.)
CKPT_KEY_PREFIX = ""
...
# tools/export_tabpfn gives `_pos_base` and tools/export_tabdpt gives
# `bin_centres`.
#
# `_LINK_SCORES` is the frozen random-attention-link table from
# orion_msp_patches (see Patch 4 there). dynamo captures it as a module-global
# constant, so it arrives with a mangled name like
# "G____import_export_orion_msp_dot_orion_msp_patches____LINK_SCORES" -- matched
# by suffix rather than exactly, since the mangling is a torch implementation
# detail. At MAX_L=512 it is 1 MB, well over the >=1KB "unmatched large
# initializer" guard, which is the guard working correctly: anything else that
# size WOULD be a weight.
INLINE_CONSTANT_SUFFIXES = ("_LINK_SCORES",)


def _is_inline_constant(name: str) -> bool:
    return name.endswith(INLINE_CONSTANT_SUFFIXES)
...
def build_tensor_map(model_proto: onnx.ModelProto, state_dict: dict,
    ...
    for init in model_proto.graph.initializer:
        if _is_inline_constant(init.name):
