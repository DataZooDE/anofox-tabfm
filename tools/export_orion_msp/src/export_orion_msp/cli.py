"""CLI: uv run export_orion_msp --config real --out DIR.

Writes into --out:
  graph_orion_msp_classification.onnx       weight-free graph (checkpoint
                                            initializers are EXTERNAL stubs; the
                                            .onnx.data is deleted)
  tensor_map_orion_msp_classification.json  ONNX initializer name -> safetensors key
  export_report_orion_msp_classification.json  provenance + parity numbers

Orion-MSP is classification-only (upstream ships no regression head), so unlike
...
def main(argv=None) -> int:
    ap = argparse.ArgumentParser(prog="export_orion_msp")
    ...
    graph_path = out / f"graph_orion_msp_{TASK}.onnx"
    map_path = out / f"tensor_map_orion_msp_{TASK}.json"

    print(f"[export_orion_msp] building random-weight OrionMSP ({args.config}) ...", flush=True)
    t0 = time.time()
    model = build_model(cfg.model_kwargs, seed=args.seed)
    n_params = sum(p.numel() for p in model.parameters())
    print(f"[export_orion_msp] {n_params:,} params ({time.time()-t0:.1f}s)", flush=True)

    t0 = time.time()
    wrapper = export.export_graph(model, graph_path, dim_rows=cfg.dim_rows,
                                  dim_train=cfg.dim_train, dim_features=cfg.dim_features,
                                  example=cfg.example)
    print(f"[export_orion_msp] dynamo export done ({time.time()-t0:.1f}s)", flush=True)

    t0 = time.time()
    tensor_map = export.postprocess(graph_path, dict(model.state_dict()))
    print(f"[export_orion_msp] postprocess done ({time.time()-t0:.1f}s): "
    ...
    if not args.skip_parity:
        ...
        print(f"[export_orion_msp] parity ({time.time()-t0:.1f}s): worst {parity['worst']:.2e} "
              f"(budget {parity['tol']:.0e}) -> {'OK' if parity['ok'] else 'FAIL'}", flush=True)
        if not parity["ok"]:
            print("[export_orion_msp] PARITY FAILED", file=sys.stderr)
            return 1
    ...
    (out / f"export_report_orion_msp_{TASK}.json").write_text(json.dumps(report, indent=2) + "\n")

    print(f"[export_orion_msp] graph: {graph_path} "
          f"({graph_path.stat().st_size/1e6:.2f} MB, weight-free)")
    print(f"[export_orion_msp] map:   {map_path}")
    return 0
