"""Orion-BiX -> weight-free ONNX graph + tensor-name map.

Mirrors ``tools/export_tabicl`` step for step (Orion-BiX is a TabICL descendant),
...
from export_orion_bix.orion_bix_patches import ExportWrapper

OPSET = 18
PARITY_TOL = 1e-3  # budget

# Orion-BiX is classification-only (no upstream regression head), so y always
...
# The released Orion-BiX checkpoint was saved from a torch.compile-wrapped
