...
|---|---|---|---|---|
| S1 | ✅ **done — with a humbling result.** On an A40 with the fixture workload: fp32 (use_tf32=0) max_rel `7.9427e-07`, mean `7.552 ms`; tf32 max_rel `7.9427e-07`, mean `7.622 ms` — **identical agreement and identical speed within noise**. At fixture scale (T=5, H=8) the matmuls are too small for tensor cores to engage, so TF32 rounding never bites and the `tf32` mode's *benefit* is unproven; its cost was already shown to be zero here. The strict-fp32 flag stays (correctness must not depend on cuBLAS kernel-selection luck), but the plan stops implying a measured TF32 speedup — that claim needs the real 6.6 GB model at large T, queued for a warm pod | timed 20-run sweep per mode via the checked-in verify host | done | `tf32` stays offered as an opt-in with honest "no measured benefit at small scale" framing |
...
| S3 | ✅ **done — hypothesis FALSIFIED.** The 42 flips are NOT near-ties: flipped rows' CPU margins run min 0.0098 / median 0.23 / max **0.805**, and 37/42 exceed 5% — bf16 occasionally overturns predictions the fp32 reference holds with high confidence (unflipped rows' median margin 0.99, same run, `SERVED_BY=rocm:0`). Proportion kept honest: 1.7% of rows on adversarially synthetic data, zero flips on the realistic CSV workflow, and fp32-by-default already protects. Consequence shipped: every "near-tie" claim in settings/README replaced with the measured statement ("rare but not confined to near-ties; validate on your data"). Open follow-up: whether fp16 (more mantissa, less exponent) behaves better on the same rows | margin-vs-flip join over the 2500-row scale run | done | bf16 stays offered, with an honest warning instead of a soothing one |
...
| S5 | ✅ **partially done, and it changes P8.** The 15-line dlopen repro (providers_shared then providers_cuda from a non-GetRuntimePath directory) that SIGSEGVs in a global ctor on 1.28.0 **does not crash on the 1.29.0 wheel** (exit 0 on an A40; whether it loads cleanly or fails with a clean dlerror was lost to an over-aggressive output filter — the verdict survives, the mechanism does not, recheck when filing). Consequences: (a) the P8 null-deref filing must be re-scoped to "affects ≤1.28.x", if filed at all; (b) upgrading the pinned runtime wheel 1.28.0 → 1.29.0 gains a crash-class fix and becomes the concrete phase-2 target; (c) the official CUDA Plugin EP against a static core remains untested — still open | newest-wheel repro re-run on the same pod session as S1 | mostly done | P8 filings re-scoped; phase-2 target named |
...
| S7 | ✅ **done** — `runpod_run.py` grew volume verbs (`--create-volume`/`--list-volumes`/`--delete-volume`, `--volume-id` mounts at `/workspace` and pins the pod to the volume's datacenter). Persistence proven the only way that counts: a marker written by one pod, read by a **fresh** pod after the first terminated (`S7_PERSISTENCE_PROVEN`, RTX 4090, EUR-IS-1). Hard-won operational facts now encoded or recorded: network volumes exist only in specific DCs (the API's 500 lists them), GPU stock is DC-scoped and scarce (a GraphQL `lowestPrice(dataCenterId)` probe found EUR-IS-1/EUR-NO-1 as the only volume-capable DCs with stock that day), and the volume's DC pins the pod, so capacity misses surface as the usual fallback. **Live volume: `4gmcwywto3` (40 GB, EUR-IS-1, ~$2.80/month recurring — delete with `--delete-volume 4gmcwywto3` when the CUDA work pauses).** Next uses: seed it with the toolchain + ORT archive + weights so pod iterations skip setup entirely | two-pod marker round-trip | done | the fast CUDA loop exists |
