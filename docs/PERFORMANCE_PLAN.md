...
| #4 precompile `.mxr` (off the query path) | **implemented + verified** — `CALL tabfm_gpu_precompile(task [, rows, features])` warms the shape-bucket: on ROCm it runs+caches the MIGraphX compile that otherwise stalls the *first predict* for ~27 min; on CPU/CUDA it warms the ORT session. Verified: reuses a cached `.mxr` (20 s load) instead of recompiling. A *hosted* `.mxr` download channel (skip the compile entirely) remains a further, infra-gated step. |
