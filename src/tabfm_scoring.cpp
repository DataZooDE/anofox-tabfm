...
//===----------------------------------------------------------------------===//
// DistributionQuantile — inverse CDF via cumsum search + linear interpolation.
//
// Inlined from tabfm_engine.cpp (Phase 2 adds external linkage there; we keep
// a local copy here so tabfm_scoring.cpp has no dependency on tabfm_engine.cpp
// or the Phase-2 tabfm_predict.hpp declaration).
//
// Spec ref: PSR-03 (used by ComputeIntervalScore for quantile bounds l/u)
//===----------------------------------------------------------------------===//
double DistributionQuantile(const std::vector<double> &probs, const std::vector<double> &borders, double q) {
...
double ComputeIntervalScore(double y, const std::vector<double> &probs,
	...
	const double l = DistributionQuantile(probs, borders, q_lo);
	const double u = DistributionQuantile(probs, borders, q_hi);
